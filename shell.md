...
substitutions='
    s/\/\/.*//                        # this will replace all C comments
    s/"[^"]"/s/g
    s/[a-zA-Z_][a-zA-Z0-9_]*/v/g'     # replace all variables with the same name
...
do
    ...
    do
        ...
        sed "$substitutions" "$file1"|sort >$TMP_FILE1      # put the output to the temporary file
        sed "$substitutions" "$file2"|sort >$TMP_FILE2

        if diff -i -w $TMP_FILE1 $TMP_FILE2 >/dev/null      # redirecting outputs to /dev/null
        then
